...
def test_alpha_plot(sample_tables_dict, sample_metadata):
    ...
    alpha_df = alpha_diversity_parametrized(
        sample_tables_dict, table_name, sample_metadata
    )
    bars = ax.patches
...
def test_beta_plot(sample_tables_dict):
    ...
    beta_df = beta_diversity_parametrized(
        sample_tables_dict[table_name], taxon=taxon, metric="braycurtis"
    )
...
def test_beta_plot_pc(sample_tables_dict, sample_metadata):
    ...
    fig_pane = beta_plot_pc(
        sample_tables_dict, sample_metadata, table_name, factor, taxon
    )
