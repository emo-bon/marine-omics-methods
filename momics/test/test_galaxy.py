...
def bcgalaxy():
    ...
    """
    with patch.dict(os.environ, {"GALAXY_URL": "http://fake-galaxy-instance", "GALAXY_API_KEY": "fake_api_key"}):
...
def test_init(bcgalaxy):
    ...
    """
    assert bcgalaxy.url == "http://fake-galaxy-instance", "The Galaxy URL should be set correctly"
    assert bcgalaxy.api_key == "fake_api_key", "The Galaxy API key should be set correctly"

def test_get_histories(monkeypatch, bcgalaxy):
    """
    Tests the get_histories method.
    """
    mock_histories = [{"id": "1", "name": "History 1"}, {"id": "2", "name": "History 2"}]
...
def test_create_history(monkeypatch, bcgalaxy):
    ...
    assert bcgalaxy.history_name == "New History", "The new history name should be set correctly"
...
def test_set_existing_history(monkeypatch, bcgalaxy):
    ...
    assert bcgalaxy.history_name == "Existing History", "The existing history name should be set correctly"

def test_get_datasets_by_key(monkeypatch, bcgalaxy):
    """
    Tests the get_datasets_by_key method.
    """
    mock_datasets = [{"name": "Dataset 1", "key": "value"}, {"name": "Dataset 2", "key": "value"}]
    ...
    assert datasets == ["Dataset 1", "Dataset 2"], "The datasets should be retrieved correctly by key and value"
...
def test_upload_file(monkeypatch, bcgalaxy):
    ...
    assert bcgalaxy.dataset_id == "dataset_id", "The dataset ID should be set correctly after upload"